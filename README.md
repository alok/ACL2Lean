...
- **Executable replay seeds**: resolved dynamic `:USE` targets now surface as concrete replay-seed tactics in `acl2lean hints` and `ACL2Lean.ProofMode`; `ACL2Lean.Tactics.acl2_use` handles plain imported theorem names, while `acl2_use_instance` replays imported theorems with ACL2-style instance bindings such as `((n (+ n 1)))`.
