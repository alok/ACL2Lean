...
- **Dynamic payload normalization**: Lean-side consumers now parse dynamic ACL2 `:IN-THEORY` action payloads back into the shared `TheoryExpr` model, and they also reparse dynamic `:EXPAND` plus `:DO-NOT-INDUCT` payloads into structured ACL2 terms, so emitted replay guidance shows up in `acl2lean hints ...` output and in the proof-mode notes/checkpoint views as more than opaque strings.
