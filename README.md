...
- **Executable replay seeds**: resolved dynamic `:USE` targets now surface as concrete `acl2_use "..."` replay-seed tactics in `acl2lean hints` and `ACL2Lean.ProofMode`, and `ACL2Lean.Tactics.acl2_use` resolves those ACL2 theorem names through the imported-theorem registry before trying the matching Lean theorem(s).
