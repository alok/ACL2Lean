...
- **Executable replay seeds**: resolved dynamic `:USE` targets now surface as concrete replay-seed tactics in `acl2lean hints` and `ACL2Lean.ProofMode`; `ACL2Lean.Tactics.acl2_use` handles plain imported theorem names, `acl2_use_instance` replays imported theorems with ACL2-style instance bindings such as `((n (+ n 1)))`, and `acl2_induct_term` now replays ACL2-selected induction calls directly from emitted ACL2 syntax such as `"(CLOG2 N)"` when the translated Lean definition exposes an induction principle.
