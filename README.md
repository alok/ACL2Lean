...
- **Structured replay actions**: the dynamic bridge now turns `:USE` hint-events, splitter notes, warning-driven disable advice, non-recursive-definition warnings, rewrite-rule subsumption warnings, and ACL2 induction choices into typed candidate replay actions that both the CLI and proof-mode panel can surface.
