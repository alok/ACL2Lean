...
- **Dynamic theory normalization**: Lean-side consumers now parse dynamic ACL2 `:IN-THEORY` action payloads back into the shared `TheoryExpr` model, so real emitted theory guidance shows up as structured `enable` / `disable` items in `acl2lean hints ...` output and in the proof-mode rune/notes view instead of remaining an opaque string.
