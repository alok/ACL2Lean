...
- **Structured replay actions**: the dynamic bridge now turns `:USE` hint-events, warning-driven disable advice, rewrite-rule subsumption warnings, and ACL2 induction choices into typed candidate replay actions that both the CLI and proof-mode panel can surface.
