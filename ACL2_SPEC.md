...
- That `flog2` extension is directly useful for the hint-generator path: ACL2's dynamic `:USE NBR-CALLS-FLOG2-UPPER-BOUND` guidance for `nbr-calls-flog2-is-logarithmic` now has a matching imported Lean theorem rather than only a parsed action record.
