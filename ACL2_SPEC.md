...
- `scripts/acl2_hint_bridge.py`, `ACL2Lean/HintBridge.lean`, and `lake exe acl2lean hints <file> <theorem>` now lift lightweight ACL2 `Goal'` / `Subgoal ...` trace lines into structured dynamic checkpoints, and the bridge now trims transcript slices to the target theorem's own summary block so warnings/observations do not bleed across neighboring generated theorems inside a shared ACL2 prompt.
