...
- `scripts/acl2_hint_bridge.py`, `ACL2Lean/HintBridge.lean`, and `lake exe acl2lean hints <file> <theorem>` now lift lightweight ACL2 `Goal'` / `Subgoal ...` trace lines into structured dynamic checkpoints, trim transcript slices to the target theorem's own summary block, and extract typed replay actions from dynamic `:USE` hint-events, splitter notes, disable warnings, non-recursive-definition warnings, rewrite subsumption warnings, and induction summaries.
