...
- `scripts/acl2_hint_bridge.py`, `ACL2Lean/HintBridge.lean`, and `lake exe acl2lean hints <file> <theorem>` now lift lightweight ACL2 `Goal'` / `Subgoal ...` trace lines into structured dynamic checkpoints, capture lifecycle/progress lines such as induction-push and checkpoint/subproof completion notices, normalize prompt-adjacent markers such as `ACL2 !>>Goal'`, keep multiline splitter/hint payloads grouped instead of splitting them into fake entries, trim transcript slices to the target theorem's own summary block, preserve full induction summaries through the emitted subgoal-count line, recover theorem-local `:HINTS` directives from ACL2-emitted `DEFTHM` transcript forms when summary `Hint-events:` are absent, preserve transcript-echoed theorem-level directives such as `:OTF-FLG` as replay-visible `transcript-option` actions when ACL2 exposes them, preserve the original ACL2 goal target for transcript-recovered hint actions, preserve checkpoint-local targets on splitter-note actions such as `Splitter note ... for Goal''`, split multi-item ACL2 `:USE` payloads into one replay action per cited theorem or instance, and extract typed replay actions from dynamic `:USE`, `:IN-THEORY`, `:EXPAND`, `:DO-NOT-INDUCT`, `:CLAUSE-PROCESSOR`, splitter notes, `:TYPED-TERM` observations, free-variable warnings, disable warnings, non-recursive-definition warnings (including plural definition lists, free-variable-search/non-rec combinations, non-`:REWRITE` rule classes, and forward-chaining trigger-term warnings), rewrite subsumption warnings (including quoted ACL2 rune names and plural prior-rule lists), goal-targeted `:USE` guidance inferred from ACL2 `[Use]` warnings, and induction summaries. Lean-side consumers now parse dynamic `:IN-THEORY` action payloads back into `TheoryExpr`, reparse dynamic `:USE` action payloads into theorem-vs-instance structure with explicit instance bindings, refine theorem-local `:USE` actions with source-hint instance bindings when ACL2’s summary only reports the base theorem name, normalize `Rules:` summary entries into structured summary-rule data, reparse dynamic `:EXPAND` / `:DO-NOT-INDUCT` payloads into structured ACL2 terms, surface dynamic clause-processor plus induction payloads as structured `clause-processor`, `induct-term`, and `induction-rule` items, surface transcript-echoed `:OTF-FLG` payloads as structured `otf-flg` items, and decode warning-derived actions such as disable-rule, disable-definition, free-variable binding, and rewrite-overlap payloads back into dedicated Lean-side structure, so the CLI and proof-mode views can inspect emitted replay guidance without re-parsing flat display strings.
- `ACL2Lean.HintBridge.DynamicArtifact.replayState` now interprets those parsed actions into a replay-oriented Lean summary: theory timeline, use timeline, split timeline, selected induction, `do-not-induct`, `otf-flg`, clause-processors, expands, cases, and typed-term foci. `lake exe acl2lean hints ...` prints that summary under `replay-state:`, and `ACL2Lean.ProofMode` threads it into the dynamic snapshot goal/rune/note panes while exposing structured `action-splitter`, `action-split-term`, and `action-typed-term` notes for downstream replay consumers.
- `ACL2Lean.HintBridge.DynamicArtifact.runeProfile` now interprets ACL2 `Rules:` summaries and concrete theory enable/disable guidance into a Lean-visible rune profile. That profile keeps the original ACL2 rule-class buckets, distinguishes theory enables from disables, and derives heuristic `lean-simp-candidates` / `lean-grind-candidates` so later replay code can consume a compact state model instead of scraping `summary-rule ...` and `theory-step ...` strings from the UI.
- `ACL2Lean.Tactics.acl2_use "<acl2-theorem>"` is now the first registry-backed replay executor on the dynamic path: it resolves ACL2 theorem names through `ImportedRegistry` and tries the matching Lean theorem(s), while `lake exe acl2lean hints ...` prints matching `replay-seed-tactics:` lines when a dynamic `:USE` action already resolves to an imported theorem. `ACL2Lean.Tactics.acl2_use_instance "<acl2-theorem>" with "<bindings>"` extends that path to imported theorem instances using ACL2-style binding lists such as `((n (+ n 1)))`, and `ACL2Lean.Tactics.acl2_induct_term "<acl2-call>"` now replays ACL2-selected induction terms directly from emitted ACL2 syntax such as `"(CLOG2 N)"` when the translated Lean definition exposes an induction principle.
...
- `ACL2Lean/Imported/Log2Replay.lean` is the first checked imported-theorem bundle in the repo: it proves ACL2 theorems `natp-clog2`, `posp-clog2`, `clog2-is-correct-lower`, `clog2-is-correct-upper`, `clog2-is-correct`, `nbr-calls-clog2=1+clog2`, `nbr-calls-flog2-lower-bound`, `nbr-calls-flog2-upper-bound`, and `nbr-calls-flog2-is-logarithmic` from `acl2_samples/2009-log2.lisp` as Lean theorems over the `SExpr` encoding, using a proof-friendly `Nat` mirror for the positive-integer path.
- That `flog2` extension is directly useful for the hint-generator path: ACL2's dynamic `:USE NBR-CALLS-FLOG2-UPPER-BOUND` guidance for `nbr-calls-flog2-is-logarithmic` now has a matching imported Lean theorem rather than only a parsed action record, the repo checks a concrete replay seed for that exact oracle step with `acl2_use "nbr-calls-flog2-upper-bound"`, and the same dynamic path can now emit candidate induction replay seeds via `acl2_induct_term` when ACL2 selects an induction term.
...
2. Execute the interpreted dynamic replay state against Lean-side goals instead of only rendering it, starting with `:USE`, theory-disable suggestions, and induction candidates.
