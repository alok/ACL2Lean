...
3. Interpret the structured `ProofInstruction` tree for a small replay prototype instead of only printing it, starting with `quiet!`, `:bash`, `:in-theory`, and `:repeat :prove`.
4. Feed extracted theorem metadata and theory events into proof replay and the ACL proof-mode widget instead of keeping them CLI/translator-only.
