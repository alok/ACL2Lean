...
- `ACL2Lean.HintBridge.DynamicArtifact.replayState` now interprets those parsed actions into a replay-oriented Lean summary: theory timeline, use timeline, selected induction, `do-not-induct`, `otf-flg`, clause-processors, expands, cases, and typed terms. `lake exe acl2lean hints ...` prints that summary under `replay-state:`, and `ACL2Lean.ProofMode` threads it into the dynamic snapshot goal/rune/note panes.
